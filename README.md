...
- Google Apps Script: 월/화/수 자동 작업, Gmail 발송, PDF 생성
- Gmail: 가족, 지인 대상 이메일 발행
- Google Drive: PDF 리포트 아카이브
...
3. Google Sheets 샘플 데이터로 주간 리포트 초안 생성 테스트하기
4. `agent_review_log`, `automation_stage_reviews`, `change_approval_log` 탭 반영하기: 완료
5. `runWeeklyDraftPrepWorkflow()`로 주간 초안 준비 원클릭 자동화하기: 완료
6. 승인 상태일 때만 이메일 발송되도록 테스트하기
7. 자동화 발전 제안도 사용자 승인 후에만 적용되도록 테스트하기
