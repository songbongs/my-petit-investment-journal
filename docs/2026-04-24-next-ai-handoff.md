...
예약 자동화는 사용자 승인 후 1차 적용되었다. 이메일 발송과 더 큰 운영 변경은 여전히 사용자 승인 전에는 진행하지 않는다.
...
1. 이메일 발송, 예약 자동화 추가 생성/수정/삭제, 중요한 운영 변경은 사용자 승인 전에는 하지 말 것
